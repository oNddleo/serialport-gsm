...
module.exports = function(SerialPort) {
  ...
  modem.open = function(device, options, callback) {
    ...
    if (options && options.cnmiCommand && options.cnmiCommand.length) modem.cnmiCommand = options.cnmiCommand
  ...
  modem.readSMSById = function(id) {
    ...
    item.logic = (newpart) => {
      ...
      if (newpart.includes('+CMGR:')) {
        if (isNaN(newpart.split(',')[0].replace('+CMGR: ', '')) == false) {
        ...
        message = self.processMessage(newMessage, messageIndex, messageStatus)
      ...
      } else if ((newpart.toString().length >= 1) && (resultData.data[resultData.data.length - 1]) && (isNaN(resultData.data[resultData.data.length - 1].msgStatus) == true)) { //read SMS mode message
        message = {
    ...
    if (typeof callback !== 'function') {
      return new Promise((resolve, reject) => {
        modem.executeCommand(command, (result, error) => {
          console.log('result executeCommanad', result)
          if (error) {
            reject(error)
          } else {
            resolve(result)
  ...
  modem.executeNext = () => {
    ...
    modem.logger.debug(`[GSM write]: ${item.command}`)
  }
  ...
  modem.dataReceived = buffer => {
    ...
    parts.forEach(part => {
      ...
      newparts.forEach(newpart => {
        modem.logger.debug(`[GSM received]: ${newpart}`)
        ...
        if (modem.queue.length && modem.queue[0]) {
          if (modem.queue[0].command.trim() === newpart.trim()) { // Echo of command received, only process if active
            ...
            modem.logger.debug('[GSM activate message processing for]: ' + newpart)
          }
          ...
          if ((modem.queue[0].command.startsWith('AT+CMGS') || pduTest.test(modem.queue[0].command))) { // Sending of Message if with response ok.. Then Message was sent successfully.. If Error then Message Sending Failed
            ...
            if (modem.queue[0].logic && modem.queue[0].inProgress) {
              ...
              modem.logger.debug(`[GSM ignore data]: ${newpart}`)
            }
          }
          let callback
          if (returnResult && modem.queue[0].inProgress) { // Expected Result was ok or with error call back function that asked for the data or emit to listener, Execute next Command if any or Execute Next Command if TIME Out and modem did not respond
            ...
            if (modem.queue[0] && modem.queue[0].callback) {
              ...
              modem.logger.debug('[GSM callback]: ' + modem.queue[0].command)
            } else {
              callback = null
              modem.logger.debug('[GSM no callback]: ' + modem.queue[0].command)
            }
  ...
  modem.getSimInbox = (callback, timeout = 15000) => {
    ...
    item.logic = (newpart) => {
      ...
      if (newpart.includes('+CMGL:')) {
        if (isNaN(newpart.split(',')[1]) == false) {
          resultData.data.push({
